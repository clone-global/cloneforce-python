...
class TestClones:
    ...
    def test_method_list(self, client: Cloneforce) -> None:
        clone = client.v1.clones.list()
        assert_matches_type(CloneListResponse, clone, path=["response"])
...
class TestAsyncClones:
    ...
    async def test_method_list(self, async_client: AsyncCloneforce) -> None:
        clone = await async_client.v1.clones.list()
        assert_matches_type(CloneListResponse, clone, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_raw_response_list(self, async_client: AsyncCloneforce) -> None:
        response = await async_client.v1.clones.with_raw_response.list()
    ...
    async def test_streaming_response_list(self, async_client: AsyncCloneforce) -> None:
        async with async_client.v1.clones.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
